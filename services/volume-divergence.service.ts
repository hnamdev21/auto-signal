...
import {
  AlertConfig,
  VolumeAlert,
  VolumeDivergenceTracker,
  KlineData,
} from "../types/market.model";

export class VolumeDivergenceService {
  private config: AlertConfig;
  private divergenceTracker: VolumeDivergenceTracker = {};
  ...
  constructor(config: AlertConfig) {
    ...
    this.divergenceTrackerFile = path.join(
      process.cwd(),
      "/data/volume-divergence-tracker.json"
    );
  ...
  private loadDivergenceTracker(): void {
    try {
      if (fs.existsSync(this.divergenceTrackerFile)) {
        ...
        console.log("📁 Loaded volume divergence tracker from file");
      }
    } catch (error) {
      console.error("❌ Error loading volume divergence tracker:", error);
  ...
  private saveDivergenceTracker(): void {
    try {
      ...
      console.error("❌ Error saving volume divergence tracker:", error);
    }
  }
   ...
   */
  getDivergenceTrackerStatus(): VolumeDivergenceTracker {
  ...
  clearDivergenceTracker(): void {
    this.divergenceTracker = {};
    this.saveDivergenceTracker();
    console.log("🗑️ Volume divergence tracker cleared");
  }
}
